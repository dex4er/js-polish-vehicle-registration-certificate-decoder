...
import { PolishVehicleRegistrationCertificateField, PolishVehicleRegistrationCertificateFuel, PolishVehicleRegistrationCertificateFuelCode } from './polish-vehicle-registration-certificate-fields'
...
type FuelCodes = {
  [P in PolishVehicleRegistrationCertificateFuelCode]: string
}
  ...
  constructor (b64Input: string) {
    ...
    for (const key of Object.keys(fieldsDefinition) as FieldsDefinitionKeys[]) {
      ...
      if ((isNewFormat && field.newFormatIndex !== undefined) || (!isNewFormat && field.oldFormatIndex !== undefined)) {
        const newDataField = { name: field.name, description: field.description } as PolishVehicleRegistrationCertificateField
